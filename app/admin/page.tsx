  ...
  if (!isAdmin) {
    return (
      <main className="grid min-h-screen place-items-center bg-slate-50 p-6">
        <section className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-100">
          <h1 className="text-2xl font-bold text-slate-900">Akses ditolak</h1>
          <p className="mt-3 text-slate-600">Akun Anda tidak memiliki akses sebagai admin.</p>
          <p className="mt-4 text-sm text-slate-500">
            Login sebagai: {session.user.name ?? "Pengguna"} ({session.user.email})
          </p>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                {session.user.name ?? "Admin"} ({session.user.email})
              </p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <form action={logout}>
                <button className="inline-flex items-center justify-center rounded-xl bg-slate-800 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-700">
                  Logout
                </button>
              </form>
            </div>
